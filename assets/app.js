  ...
  function orgGroup(g) {
    const items = g.items
      .map((o) => {
        ...
        const inner = `${logoTile(name, o.logo, o.wide ? 'logo--wide' : '', o.short)}<span class="org__name">${name}</span>`;
  ...
  function proseBlock(b) {
    const logos = b.logos
      ? `<ul class="logo-row">${b.logos
          .map((l) => {
            const inner = `${logoTile(l.name, l.logo, l.wide ? 'logo--wide' : '', l.short)}<span class="org__name">${l.name}</span>`;
